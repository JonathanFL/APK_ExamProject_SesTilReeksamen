...
public:
    ...
    {
        std::cout << "Using GreatBall: probability of catching is " << 0.75_to_percent << "%" << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        PokeBagItemResult res;
        if(pokemon.getLevel_() > 25){// lower probability of catch if level if higher than 25
            res.result = caught(probabilityOfCatch-levelPunishment) ? caughtSuccessMessage : caughtFailMessage;
        }else{
