...
public:
    ...
    {
        std::cout << "Using PokeBall: probability of catching is " << 0.75_to_percent << "%" << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        PokeBagItemResult res;
        if(pokemon.getLevel_() > 15){// lower probability of catch if level if higher than 15
